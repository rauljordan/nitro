...
func TestDeriveSha(t *testing.T) {
	...
	for len(txs) < 1000 {
		exp := DeriveSha(txs, trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := DeriveSha(txs, trie.NewStackTrie(nil))
...
func TestFuzzDeriveSha(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		seed := rndSeed + i
		exp := DeriveSha(newDummy(i), trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := DeriveSha(newDummy(i), trie.NewStackTrie(nil))
		if !bytes.Equal(got[:], exp[:]) {
...
func TestDerivableList(t *testing.T) {
	...
	for i, tc := range tcs[1:] {
		exp := DeriveSha(flatList(tc), trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := DeriveSha(flatList(tc), trie.NewStackTrie(nil))
		if !bytes.Equal(got[:], exp[:]) {
...
func genTxs(num uint64) (Transactions, error) {
	...
	newTx := func(i uint64) (*Transaction, error) {
		...
		utx := NewTransaction(i, addr, new(big.Int), 0, new(big.Int).SetUint64(10000000), nil)
		tx, err := SignTx(utx, signer, key)
		return tx, err
	}
...
func printList(l DerivableList) {
